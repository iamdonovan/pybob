...
class GeoImg:
	...
	def __init__(self, in_filename, in_dir='.', datestr=None, datefmt='%m/%d/%y'):
		...
		if type(in_filename) is str:
			self.filename = in_filename
			self.gd = gdal.Open(self.in_dir_path + os.path.sep + self.filename)
		elif type(in_filename) is gdal.Dataset:
			self.filename = None
			self.gd = in_filename
		else:
			raise Exception('filename must be a string or a gdal Dataset')
		...
		if self.filename is not None:
			if ( datestr is not None ):
				self.imagedatetime = datetime.datetime.strptime( datestr, datefmt )
	...
	def copy(self, new_raster=None, new_extent=None, driver='MEM', filename='', newproj=None):
		...
		if self.NDV is not None:
			sn = newGdal.GetRasterBand(1).SetNoDataValue(self.NDV)
	...
	def reproject(self, dst_raster, driver='MEM', filename=''):
		...
		setproj = dest.SetProjection(dst_raster.proj)
		setgt = dest.SetGeoTransform(dst_raster.gt)
		res = gdal.ReprojectImage(self.gd, dest, self.proj, dst_raster.proj, gdal.GRA_Bilinear)	

		setNDV = dest.GetRasterBand(1).SetNoDataValue(dst_raster.NDV)
		out = GeoImg(dest)
	...
	def crop_to_extent(self, extent):
		...
		setproj = dest.SetProjection(self.proj)
		newgt = (xmin, dx, 0.0, ymax, 0.0, dy)
		setgt = dest.SetGeoTransform(newgt)
		res = gdal.ReprojectImage(self.gd, dest, self.proj, self.proj, gdal.GRA_Bilinear)	

		if self.NDV is not None:
			setNDV = dest.GetRasterBand(1).SetNoDataValue(self.NDV)
	...
	def random_points(self, Npts, edge_buffer=None):
		...
		if edge_buffer is None:
			...
			if not isinstance(self.img, np.ma.MaskedArray):
				...
				goodinds = indices[np.logical_and(np.invert(self.img.mask).reshape(-1), np.isfinite(self.img.data.reshape(-1)))]

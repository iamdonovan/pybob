...
def final_histogram(dH0, dHfinal, pp):
    ...
    dH0 = np.squeeze(np.asarray(dH0[ np.logical_and.reduce((np.isfinite(dH0), (np.abs(dH0) < np.abs(np.nanmean(dH0)) + np.nanstd(dH0) * 3)))]))
    dHfinal = np.squeeze(np.asarray(dHfinal[ np.logical_and.reduce((np.isfinite(dHfinal), (np.abs(dHfinal) < np.abs(np.nanmean(dHfinal)) + np.nanstd(dHfinal) * 3)))]))

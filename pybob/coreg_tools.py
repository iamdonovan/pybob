...
def false_hillshade(dH, title, pp):
	...
	numwid = max([len('%.1f m' % np.nanmean(dH.img)), len('%.1f m' % np.nanmedian(dH.img)), len('%.1f m' % np.nanstd(dH.img))])
	plt.annotate( 'MEAN:'.ljust(8) + ('%.1f m' % np.nanmean(dH.img)).rjust(numwid), xy=(0.65, 0.89), xycoords='axes fraction', fontsize=12, fontweight='bold', color='red', family='monospace')
	plt.annotate( 'MEDIAN:'.ljust(8) + ('%.1f m' % np.nanmedian(dH.img)).rjust(numwid), xy=(0.65, 0.80), xycoords='axes fraction', fontsize=12, fontweight='bold', color='red', family='monospace')
	plt.annotate( 'STD:'.ljust(8) + ('%.1f m' % np.nanstd(dH.img)).rjust(numwid), xy=(0.65, 0.71), xycoords='axes fraction', fontsize=12, fontweight='bold', color='red', family='monospace')
...
def coreg_fitting(xdata, ydata, sdata, title, pp):
	...
	fitfun = lambda p, x: p[0] * np.cos(np.radians(p[1]-x)) + p[2]
...
#	print p1
	...
	plt.text(20, -125, '$\Delta$x: ' + ('%.1f m' % xadj).rjust(numwidth), fontsize=12, fontweight='bold',color='red', family='monospace')
	plt.text(20, -150, '$\Delta$y: ' + ('%.1f m' % yadj).rjust(numwidth), fontsize=12, fontweight='bold',color='red', family='monospace')
	plt.text(20, -175, '$\Delta$z: ' + ('%.1f m' % zadj).rjust(numwidth), fontsize=12, fontweight='bold',color='red', family='monospace')
...
def dem_coregistration(masterDEM, slaveDEM, glaciermask=None, landmask=None, outdir='.'):
	...
	if len(masterDEM.split(os.path.sep)) == 1:
		masterpath = '.'
	else: 
		masterpath = os.path.sep.join(masterDEM.split(os.path.sep)[:-1])

	if len(slaveDEM.split(os.path.sep)) == 1:
		slavepath = '.'
	else: 
		slavepath  = os.path.sep.join(slaveDEM.split(os.path.sep)[:-1])

	masterfilename = masterDEM.split(os.path.sep)[-1]
	slavefilename  = slaveDEM.split(os.path.sep)[-1]

	masterDEM = GeoImg(masterfilename, in_dir=masterpath) 
	slaveDEM  = GeoImg(slavefilename, in_dir=slavepath)
	...
	slopecall = subprocess.Popen(' '.join(callstr), shell=True, stdout=open(os.devnull, 'wb'), stderr=subprocess.PIPE).stderr.read()
	...
	aspectcall = subprocess.Popen(' '.join(callstr), shell=True, stdout=open(os.devnull, 'wb'), stderr=subprocess.PIPE).stderr.read()
	...
	slaveoutfile = '.'.join(slavefilename.split('.')[0:-1]) + '_adj.tif'
	mastoutfile = '.'.join(masterfilename.split('.')[0:-1]) + '_resamp.tif'

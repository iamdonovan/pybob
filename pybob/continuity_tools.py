...
import numpy as np, matplotlib.pyplot as plt, geopandas as gpd, pandas as pd, os
...
def calculate_over_fluxgate(start, vdata, surfdata, bdot, dhdt, beddata, end=None, offset=100, gamma=0.9, plotfig=None):
	...
	while (startgate.length - fluxDist) > offset:
		...
		thesegates, thisbed = calculate_bed_elevs(flowLine1, flowLine2, vdata, surfdata, bdot, dhdt, beddata, gamma, offset, plotfig)
...
def calculate_bed_elevs(flowline1, flowline2, vdata, surfdata, bdot, dhdt, beddata, gamma, offset, plotfig=None):
	...
	for j,c in enumerate(cells):
		...
		thickness = np.append(thickness, (qup - rhs) / (np.dot(np.array(ux[j+1],uy[j+1]),c.n_dn) * widths[j+1] * gamma) )
		#thickness = np.append(thickness, (qup - rhs) / (np.sqrt(ux[j+1]**2 + uy[j+1]**2) * widths[j+1] * gamma) )
	 	bed = np.append(bed, gatesurf[j+1] - thickness[-1]) # surface at downstream boundary minus calculated thicknes (at downstream bndry)
...
def get_flowline( start, vdata, maxt=5000, end=None ):
	t = np.arange(0, maxt / vdata.unitConv, 10 / vdata.unitConv) # should figure out what the realtionship between t, ground distance is...
...
def linedistance(x,y): #re-write this with shapely stuff!
	tmp = LineString(zip(x,y))
	lined = [tmp.project(Point(p)) for p in zip(x,y)]
...
def flowbandwidth(flowline1, flowline2, offsets):
	...
	for i, pt in enumerate(flowline1.x):
		widths = np.append(widths, Point(flowline1.x[i],flowline1.y[i]).distance(Point(flowline2.x[i],flowline2.y[i])) )
	return widths
